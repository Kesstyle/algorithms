...
package pl.kes.algorithms.book.standard;
 ...
 *  an image using the ARGB color format.
   ...
   /**
    ...
    public void save(String name) {
  	...
  	if (name.length() == 0) throw new IllegalArgumentException("argument to save() is the empty string");
        ...
        if ("jpg".equalsIgnoreCase(suffix) && hasAlpha()) {
            System.out.println("Error: filename must end in .png because the JPEG format does not support transparency");
            return;
        }
